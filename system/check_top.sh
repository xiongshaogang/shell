...
FLOW(){
in_flow1=$(cat /proc/net/dev | grep lo | sed -e "s/(.*):(.*)/[1]/g" | awk ' { print $2 }')
echo $in_flow1
in_flow2=$(cat /proc/net/dev | grep eth0 | sed -e "s/(.*):(.*)/[1]/g" | awk ' { print $2 }')
echo $in_flow2
in_flow_byte=$(expr $in_flow1 + $in_flow2)
echo $in_flow_byte 
in_flow=$(expr $in_flow_byte / 1024)
echo $in_flow
out_flow1=$(cat /proc/net/dev | grep lo | sed -e "s/(.*):(.*)/[1]/g" | awk ' { print $10 }')
echo $out_flow1
out_flow2=$(cat /proc/net/dev | grep eth0 | sed -e "s/(.*):(.*)/[1]/g" | awk ' { print $10 }')
echo $out_flow2
