...
do
	...
	do
		...
		if [ -f "$backroot/$file_month/raw/$subpath/$file" ]; then
			echo "backup existed: $file"
			rm -f $file
			continue
		else
			backpath=$backroot/$file_month/raw/$subpath
			if [ ! -d $backpath ]; then
