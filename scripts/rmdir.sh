...
if($#argv >3 || $#argv < 1)then
	echo "Usage:command path [roam|wj|sp [nokia....]]"
	exit 1
endif	
...
if($#argv == 3) then
	set command="rmcommand.sh $3 "
else
	set command="rmcommand.sh "
endif 
echo "continue..."
...
set list2=`echo "settle dataloader/cdr dataloader/stat account"`
set dir3list=`echo $configpath/acquire $datapath/stat/statoutput $datapath/stat/skip $datapath/dataloader/input $datapath/daydataloader/input $datapath/back/daystat/output $datapath/daystat/skip`
if($#argv == 1 || $work == "wj")then
		foreach j($dirlist)
			foreach i($list)
				...
				$command $j/$i/input/wj/voice/nokiaredo
				$command $j/$i/input/wj/voice/alcatelredo
				$command $j/$i/input/wj/voice/huaweiredo
...
if($#argv == 1 || $work == "sp")then
		foreach j($dirlist)
			foreach i($list)
					...
					$command $j/$i/output/sp/billing/sms
					$command $j/$i/output/sp/billing/mms
					$command $j/$i/output/sp/billing/wap
					$command $j/$i/output/sp/billing/kjava
					$command $j/$i/output/sp/dream
		...
		foreach j($dir3list)
			...
			$command $j/sp/billing/sms
			$command $j/sp/billing/mms
			$command $j/sp/billing/wap
			$command $j/sp/billing/kjava
			$command $j/sp/dream
			$command $j/sp/bill	
		end		
		...
		foreach j($dir1list)
			foreach i($list)
					...
					$command $j/$i/sp/billing/sms
					$command $j/$i/sp/billing/mms
					$command $j/$i/sp/billing/wap
					$command $j/$i/sp/billing/kjava
					$command $j/$i/sp/dream
					$command $j/$i/sp/bill
			end		
		end		
		foreach j($dir2list)
